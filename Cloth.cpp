...
double Cloth::condBend(int t1, int t2)
{
    ...
    //dvec3 n1 = getNormTriangle(triangles[t1]);
    //dvec3 n2 = getNormTriangle(triangles[t2]);
    auto shared = sharedEdge(t1, t2);
    auto e = normalize(points[shared.first] - points[shared.second]);
...
void Cloth::addStretchXForces(double str)
{
    for(int i = 0; i < triangles.size(); i++)
    {
        ...
        //cout << "STRX " << (get<0>(gradx)*condx*KSTRX)[0] << endl;
...
void Cloth::addStretchYForces(double str)
{
    for(int i = 0; i < triangles.size(); i++)
    {
        ...
        //cout << "STRY " << (get<0>(grady)*condy*KSTRY)[0] << endl;
...
void Cloth::addShearForces()
{
    for(int i = 0; i < triangles.size(); i++)
    {
        ...
        //cout << "SHEAR " << (get<0>(gradsh)*condsh*KSH)[0] << endl;
...
void Cloth::checkAndBend(int t1, int t2)
{
    ...
    //cout << "BEND " << (get<0>(gradb)*condb*KBEND)[0] << endl;
